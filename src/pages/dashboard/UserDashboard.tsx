...
export default function UserDashboard({ onNavigate }: UserDashboardProps) {
  const { profile, becomeConsultant } = useAuth();
  ...
  const handleSwitchToCreator = async () => {
    try {
      ...
      setProfileNotice({ type: "error", text: err.message || "Could not switch to creator right now." });
    } finally {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="flex items-center justify-between px-2">
              ...
              <button onClick={() => onNavigate?.("messages")} className="text-sm font-black text-blue-600 hover:underline">View All</button>
            </div>
            ...
            {loading ? (
              ...
              <div className="bg-white p-20 rounded-[2.5rem] border border-slate-100 shadow-xl text-center">
                ...
                <button onClick={() => onNavigate?.("marketplace")} className="px-10 py-4 bg-[#0c2438] text-white font-black rounded-2xl hover:bg-[#1a3a5a] transition-all">
                  Explore Mentors
                </button>
              </div>
            ) : (
          ...
          <div className="space-y-6">
            <div className="bg-blue-600 p-8 rounded-[2.5rem] shadow-xl shadow-blue-600/20 relative overflow-hidden">
               ...
               <p className="text-white/80 font-black leading-relaxed mb-6 relative z-10">Unlock exclusive masterclasses and direct line to top mentors.</p>
                <button onClick={() => onNavigate?.("memberships")} className="w-full py-4 bg-[#0c2438] text-white font-black rounded-2xl hover:scale-105 transition-all relative z-10">
                   Upgrade Plan
                </button>
             </div>
             ...
             <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/20 space-y-4">
               <h3 className="text-xl font-black text-slate-900">Creator Tools</h3>
               <p className="text-slate-500 font-medium">Turn your profile into a creator dashboard and start listing sessions.</p>
               <button
                 onClick={handleSwitchToCreator}
                 disabled={switchingCreator}
                 className="w-full py-4 bg-blue-600 text-white font-black rounded-2xl hover:bg-blue-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50"
               >
                 {switchingCreator ? "Switching..." : "Switch to Creator"}
                 {!switchingCreator && <ChevronRight className="w-5 h-5" />}
               </button>
             </div>
           </div>
         </div>
