...
export default function ConsultantSidebar({ 
  ...
  const [desktopCollapsed, setDesktopCollapsed] = useState(() => localStorage.getItem('mentoga_consultant_nav_collapsed') === 'true');
  ...
  const NavLink = ({ item }: { item: NavItem }) => {
    ...
    return (
      <button
        ...
        title={item.label}
        className={`w-full flex items-center ${desktopCollapsed ? 'justify-center' : 'gap-3 px-4'} py-3 rounded-2xl transition-all group ${
          isActive 
      ...
      >
        ...
        {!desktopCollapsed && <span className="text-sm flex-1 text-left">{item.label}</span>}
        {item.hasBadge && (
  ...
  const toggleDesktopCollapsed = () => {
    setDesktopCollapsed((current) => {
      const next = !current;
      localStorage.setItem('mentoga_consultant_nav_collapsed', String(next));
      return next;
    });
  };
  ...
  return (
    <>
      ...
      <div className={`hidden md:flex ${desktopCollapsed ? 'w-24' : 'w-72'} h-screen flex-col bg-white border-r border-slate-100 overflow-hidden shrink-0 transition-all duration-300`}>
      {/* Brand Logo */}
      <div className={`p-6 ${desktopCollapsed ? 'px-4' : ''} flex items-center ${desktopCollapsed ? 'justify-center' : 'justify-between'}`}>
        <div className="flex items-center gap-2 min-w-0">
           <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center text-white font-black italic">M</div>
           {!desktopCollapsed && <span className="text-xl font-black text-slate-900">Mentoga</span>}
         </div>
      ...
      <div className={`flex-1 overflow-y-auto ${desktopCollapsed ? 'px-2' : 'px-3'} space-y-6 pb-20 custom-scrollbar`}>
        {/* Main Section */}
      ...
      <div className="p-4 border-t border-slate-100 bg-white relative">
        ...
        <button 
          ...
          className={`w-full flex items-center ${desktopCollapsed ? 'justify-center' : 'gap-3'} p-2 hover:bg-slate-50 rounded-2xl transition-all group`}
        >
          ...
          {!desktopCollapsed && <div className="flex-1 text-left">
          ...
          {!desktopCollapsed && <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${showProfileMenu ? 'rotate-180' : ''}`} />}
        </button>
      </div>
